...
### 2026-05-06 Phase 0 第六批

- 已让 `DECSET ?1049` 每次进入备用屏都创建干净缓冲区，避免上一次 TUI 画面残留。
- 已禁用备用屏 scrollback 记录，避免 vim、less、fzf 等全屏 TUI 的内部滚动污染历史输出。
- 已保留主屏 scrollback 默认行为，并用 `ScrollbackEnabled` 明确区分主屏和备用屏策略。
- 已增加备用屏重新进入清屏、备用屏滚动不记录 scrollback 的回归测试。

### 2026-05-06 Phase 0 第七批

- 已支持 legacy `DECSET ?47` / `DECRST ?47` 切换备用屏和主屏。
- 已支持 xterm `DECSET ?1048` / `DECRST ?1048` 在不切换屏幕的情况下保存和恢复光标。
- 已让 `?47` 复用备用屏干净缓冲区和禁用备用屏 scrollback 的策略。
- 已增加 `?47` 切屏、`?1048` 保存恢复的回归测试。

### 2026-05-06 Phase 0 第八批

- 已默认启用 DECAWM 自动换行，长输出到达右边界后进入下一行，不再覆盖最后一列。
- 已让 CJK/emoji 等宽字符在最后一列前提前换行，避免被截断或破坏 spacer cell。
